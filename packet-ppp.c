...
#include "headers/emem.h"
#include "headers/packet-ppp.h"
#include "headers/ppptypes.h"
#include "headers/etypes.h"
#include "headers/ip_opts.h"
#include "headers/atalk-utils.h"
#include "headers/packet-chdlc.h"
#include "headers/packet-ip.h"
#include "headers/packet-ipx.h"
#include "headers/packet-vines.h"
#include "headers/nlpid.h"
#include "headers/crc16-tvb.h"
#include "headers/crc32-tvb.h"
#include "headers/ipproto.h"
#include "headers/addr_resolv.h"
#include "headers/packet-usb.h"
#include "headers/packet-sll.h"
#include "headers/fakefunctions.h"
#include "headers/tvbuff.c"
#include "headers/except.c"
...
decode_fcs(tvbuff_t *tvb, proto_tree *fh_tree, int fcs_decode, int proto_offset)
{
    ...
    case NO_FCS:
        next_tvb = tvb_new_subset_rem(tvb, proto_offset);
        break;
    ...
    case FCS_16:
        ...
        if (reported_len < 2 || len < 0) {
            ...
            if (len > reported_len)
                len = reported_len;
            next_tvb = tvb_new_sub(tvb, proto_offset, len, reported_len);
        } else {
            /*
...
/*
    ...
    case PROTREJ:       /* LCP only: RFC 1661 */
        ...
        if (length > 2) {
            ...
            next_tvb = tvb_new_sub(tvb, offset, length, length);
    ...
    proto_tree *fh_tree, proto_item *ti, int proto_offset)
{
    ...
    next_tvb = tvb_new_subset_rem(tvb, proto_len);
...
dissect_vsnp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    next_tvb = tvb_new_subset_rem(tvb, 1);
...
dissect_bcp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    if (!(flags & BCP_IS_BCONTROL)) {
        ...
        if (reported_length >= pad_length) {
            ...
            next_tvb = tvb_new_sub(tvb, offset, captured_length,
                reported_length);
            switch (mac_type) {
...
dissect_mp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    if (tvb_reported_length_remaining(tvb, hdrlen) > 0) {
        next_tvb = tvb_new_subset_rem(tvb, hdrlen);
...
dissect_ppp_usb( tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree )
{
    ...
    if ((tvb_memeql(tvb, 0, buf2, sizeof(buf2)) == 0) ||
        ...
        if (tvb_get_guint8(tvb, 1) == 0x03)
            next_tvb = tvb_new_subset_rem(tvb, 2);
        else
            next_tvb = tvb_new_subset_rem(tvb, 3);
        dissect_ppp(next_tvb, pinfo, tree);
...
{
    const struct true_false_string tfs_yes_no;
...
{
    struct true_false_string tfs_yes_no;
    static hf_register_info hf[6];
    /*
    static hf_register_info hf[] = {
        { &hf_bcp_flags,
...
{
    const struct true_false_string tfs_enabled_disabled;
    static hf_register_info hf[] = {
        { &hf_ccp_opt_type,
...
int main(){
	...
	dissect_ccp_bsdcomp_opt(opt, tvb, offset, length, pinfo , tree);
